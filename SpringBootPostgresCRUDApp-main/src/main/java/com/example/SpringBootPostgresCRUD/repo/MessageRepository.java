...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT message FROM Message message WHERE message.userSender.email= :email or message.userReceiver.email=:email")
    public List<Message> findBySenderOrReceiverEmail(@Param("email") String email);

    @Query("SELECT message FROM Message message WHERE (message.userReceiver.email= :email1 and message.userSender.email=:email2) or (message.userReceiver.email= :email2 and message.userSender.email=:email1)")
    public List<Message> findByPreviousChat(@Param("email1") String email1, @Param("email2") String email2);

    @Query("SELECT message FROM Message message WHERE message.userReceiver.email= :email1 and message.userSender.email=:email2")
